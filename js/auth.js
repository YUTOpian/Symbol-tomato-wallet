...
/* ============================================================
   ログアウト
   保存済みボールト(localStorage/sessionStorage)自体は削除しない。
   ホーム画面の確認ダイアログで「アカウントの情報はこの端末に保存された
   ままなので、次回はようこそ画面で同じパスワードを入力すればログイン
   できます」と案内しているため、ここでボールトを消してしまうと
   その案内と矛盾し、ようこそ画面が「保存データなし」の表示になって
   しまう(パスワード欄・ログインボタン・削除ボタンが消える不具合の原因)。
   保存済みアカウントデータそのものを削除したい場合は、ようこそ画面の
   「アカウントデータを削除する」ボタン(clearVault)を使うこと。
   メモリ上の鍵材料(sessionSalt/sessionKey)はセキュリティのため破棄する。
============================================================ */
function logout() {
  sessionSalt = null;
  sessionKey = null;
