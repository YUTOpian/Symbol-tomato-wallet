...
function renderSummary(results, combinedResult) {
  ...
  const totalTruncated = okResults.some((r) => r.result.truncated) || combinedResult.truncated;
...
async function loadExchangeFlowAnalysis(mode, customRange) {
  ...
  try {
    ...
    // 追跡対象の全アドレス(7つ)を最初から1つのグループとみなしてスキャンする
    // (scanExchangeGroupに全アドレスをまとめて渡すことで、取引所の垣根を
    //  越えたアドレス同士の移動もすべて「内部移動」として除外できる。
    //  これは個別取引所の集計と全く同じ、既にテスト済みのロジックを
    //  そのまま使うため、計算方法の食い違いによる誤差が発生しない)。
    ...
    const [results, combinedResult] = await Promise.all([
      ...
      (async () => {
        try {
          return await scanExchangeGroup(allAddressEntries, fromHeight, toHeight, xymMosaicIds, () => {});
        } catch (e) {
    ...
    renderSummary(results, combinedResult);
    ...
    lastExchangeFlowSummary = {
      rangeLabel,
      combinedTotals: {
        inflow: combinedResult.inflowAmount,
        outflow: combinedResult.outflowAmount,
        inflowCount: combinedResult.inflowCount,
        outflowCount: combinedResult.outflowCount,
        truncated: combinedResult.truncated,
        errored: combinedResult.errored,
      },
...
function exportExchangeFlowCsv() {
  ...
  try {
    ...
    // 計算した正しい値(集計実行時に計算済みのcombinedTotals)をそのまま使う。
